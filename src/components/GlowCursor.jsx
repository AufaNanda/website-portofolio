...
const GlowCursor = ({
  ...
  enabled = true,
  // Additive option: track the pointer across the whole window instead of
  // just this element. Lets the component be used as a fixed page overlay.
  trackWindow = false,
  children,
  className = '',
  style,
  ...
  useEffect(() => {
    ...
    const eventTarget = initialConfig.trackWindow ? window : container;
    const leaveTarget = initialConfig.trackWindow ? document.documentElement : container;
    const resizeObserver = new ResizeObserver(resize);
    resizeObserver.observe(container);
    eventTarget.addEventListener('pointermove', updatePointer);
    eventTarget.addEventListener('pointerenter', updatePointer);
    ...
    return () => {
      ...
      eventTarget.removeEventListener('pointerenter', updatePointer);
